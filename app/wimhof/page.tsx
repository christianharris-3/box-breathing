...
function getUrl(inputs: InputsType): string {
    let output = "/wimhof-breathing?"
    output += "breathCount="+inputs.breathCount
    output += "&recoveryLength="+inputs.recoveryLength
    output += "&breathingRate="+inputs.breathingRate
    output += "&holdDurations="+inputs.holdDurations.join(",")

    return output
}

export default function BreathingInputs(){//{doneFunction}:{doneFunction: (arg0: InputsType)=>void}) {
    ...
    return (
        <div className="flex flex-1 justify-center bg-green-300 transition duration-3000 ease-in-out">
            <div className="bg-blue-300 text-gray-800 font-semibold mt-10 h-full p-5 rounded-xl shadow-2xl">
                ...
                <div className="shadow-md rounded-2xl bg-blue-200 pt-2">
